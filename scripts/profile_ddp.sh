...
torchrun --standalone --nproc_per_node="${NPROC}" "${EXP}" --mode new --max-steps "${MAX_STEPS}" --run-label "${PLAIN_LABEL}"
...
if command -v nsys >/dev/null 2>&1; then
  if ls "${NSYS_PREFIX}"* >/dev/null 2>&1; then
    ...
    nsys profile \
      ...
      torchrun --standalone --nproc_per_node="${NPROC}" "${EXP}" --mode new --max-steps "${PROFILE_MAX_STEPS}" --run-label "${NSYS_LABEL}"
    nsys_status=$?
    set -e
